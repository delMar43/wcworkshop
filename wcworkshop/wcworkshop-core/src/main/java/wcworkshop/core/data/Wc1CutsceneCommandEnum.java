...
public enum Wc1CutsceneCommandEnum {
  //@formatter:off
  ALWAYS((byte) 0),
  DAMAGE_ABOVE((byte)1),
  DAMAGE_BELOW((byte)2),
  ...
  public static Wc1CutsceneCommandEnum getByValue(byte value) {
    for (Wc1CutsceneCommandEnum cmd : values()) {
      if (value == cmd.value) {
        return cmd;
      }
    }
  ...
  private Wc1CutsceneCommandEnum(byte value) {
    this.value = value;
  }
}
