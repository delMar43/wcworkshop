    ...
    <li class="project=${projectNode.id}">${projectNode.label}
      <ul>
        <c:forEach items="${projectNode.seriesNodes}" var="seriesNode" varStatus="seriesNodeStatus">
          <li class="series=${seriesNode.id} project=${projectNode.id} label=${seriesNode.tabLabel}">${seriesNode.label}
            <ul>
            <c:forEach items="${seriesNode.missionNodes}" var="missionNode" varStatus="missionNodeStatus">
              <li class="mission=${missionNode.id} project=${projectNode.id} label=${missionNode.tabLabel}">${missionNode.label}
                <ul>
                  <li>Cutscenes
                    <ul>
