...
var submitCutsceneEditForm = function() {
  $.ajax({
    type: "POST",
    url: $("#cutsceneEditForm_${cutsceneId}").attr("action"),
    data: $("#cutsceneEditForm_${cutsceneId}").serialize(),
...
<div class="scrollablePane">
  ...
  <c:forEach items="${command.screens}" var="screen" varStatus="screenStatus">
        <tr>
          ...
          <td>
            ...
            <%-- input name="screens[screenStatus.index].commands" value="${command.screens[screenStatus.index].commands" --%>
          </td>
  ...
  <button onclick="submitCutsceneEditForm()">Save</button>
</div>