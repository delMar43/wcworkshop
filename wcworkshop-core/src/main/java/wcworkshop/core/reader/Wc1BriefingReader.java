...
public class Wc1BriefingReader {
  ...
  private Wc1MissionSlot extractMissionBlock(byte[] missionBuffer) {
    ...
    result.setBriefingCutscene(extractCutscene(missionBuffer, secondaryTable, (byte) 0, "Briefing")); // briefing
    result.setDebriefingCutscene(extractCutscene(missionBuffer, secondaryTable, (byte) 2, "Debriefing")); // debriefing
    result.setShotglassCutscene(extractCutscene(missionBuffer, secondaryTable, (byte) 4, "Shotglass")); // shotglass
    result.setLeftCutscene(extractCutscene(missionBuffer, secondaryTable, (byte) 6, "Right Bar Seat")); // right bar seat
    result.setRightCutscene(extractCutscene(missionBuffer, secondaryTable, (byte) 8, "Left Bar Seat")); // left bar seat

    return result;
  }

  private Wc1Cutscene extractCutscene(byte[] missionBuffer, List<Integer> secondaryTable, byte firstOffset, String title) {
